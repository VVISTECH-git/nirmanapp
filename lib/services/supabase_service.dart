...
class SupabaseService {
  ...
  static Future<Task> createTask(Map<String, dynamic> data) async {
    final res = await _client.from('tasks').insert({...data, 'created_by': currentUser!.id}).select().single();
  ...
  static Future<Issue> createIssue(Map<String, dynamic> data) async {
    final res = await _client.from('issues').insert({...data, 'reported_by': currentUser!.id}).select().single();
  ...
  static Future<DailyLog> upsertDailyLog(Map<String, dynamic> data) async {
    final res = await _client.from('daily_logs').upsert(
        {...data, 'logged_by': currentUser!.id}, onConflict: 'project_id,log_date').select().single();
  ...
  static Future<Expense> createExpense(Map<String, dynamic> data) async {
    final res = await _client.from('expenses').insert({...data, 'paid_by': currentUser!.id}).select().single();
    return Expense.fromJson(res);
  }

  static Future<Map<String, int>> getExpenseSummary(String projectId) async {
    final res = await _client.from('expenses').select('category, amount_paise, gst_paise').eq('project_id', projectId);
